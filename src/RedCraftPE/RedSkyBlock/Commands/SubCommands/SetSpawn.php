...
use pocketmine\player\Player; // Changed from CommandSender to Player
...
class SetSpawn {
  ...
  public function onSetSpawnCommand(Player $sender): bool {
    $this->NCDPrefix = SkyBlock::getInstance()->NCDPrefix;

    if ($sender->hasPermission("skyblock.setspawn")) {
        $senderName = strtolower($sender->getName());
        // Lấy vị trí của người chơi dưới dạng đối tượng Location
        $location = $sender->getLocation();
        // Lấy tọa độ x, y, z từ đối tượng Location
        $xPos = round($location->getX());
        $yPos = round($location->getY());
        $zPos = round($location->getZ());
